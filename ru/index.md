...
* [Введение](#introduction)
  * [Почему руководство](#why-a-styleguide)
  * [Дискламация](#disclaimer)
  * [Ключевые принципы](#key-principles)
* [Синтакс & Форматирование](#syntax--formatting)
  * [Строки](#strings)
  * [Числа](#numbers)
    * [Нули](#zeros)
  ...
  * [Листы](#lists)
  * [Карты](#maps)
    * [Отладка Sass карт](#debugging-a-sass-map)
  * [Набор правил CSS](#css-ruleset)
  * [Декларация сортировки](#declaration-sorting)
  * [Вложенность селекторов](#selector-nesting)
...
* [Архитектура](#architecture)
  ...
  * [Шаблон 7-1](#the-7-1-pattern)
    * [Основная директория](#base-folder)
    * [Директория компонентов](#components-folder)
    * [Директория лэйаута](#layout-folder)
    * [Диреткорая страниц](#pages-folder)
    * [Директория тем](#themes-folder)
    * [Директория утилит](#utils-folder)
    * [Директория вендоров](#vendors-folder)
...
* [Адаптивный веб-дизайн и точки остановки](#responsive-web-design-and-breakpoints)
  ...
  * [Использование медийных запросов](#media-queries-usage)
* [Переменные](#variables)
  * [Контекст](#scoping)
  * [флаг !default](#default-flag)
  * [флаг !global](#global-flag)
  * [Несколько переменных или карт кода](#multiple-variables-or-maps)
* [Расширение](#extend)
* [Миксины](#mixins)
  * [Основы](#basics)
  * [Список аргументов](#arguments-list)
  * [Миксины и вендорные префиксы](#mixins-and-vendor-prefixes)
...
Основая цель Sass — это починить недостатки CSS. CSS, как мы все знаем, не самый лучший язык в мире <sup>[citation needed]</sup>. Являясь
Sass's ultimate objective is to fix CSS's flaws. CSS, as we all know, is not the best language in the world <sup>[citation needed]</sup>. В то время как он очень прост в освоении, он может быстро стать трудноподдерживаем, особенно на больших проектах.
...
Между тем, Sass остается консервативным в отношении языка CSS.

Цель в том, чтобы не превратить CSS в полнофункциональный язык программирования; Sass только хочет помочь там, где CSS не справляется. Поэтому, начать использовать Sass не сложнее, чем начать изучать CSS. Он просто добавляет дополнительный функционал поверх CSS.

Есть много способов использовать этот функционал. Какие-то способы хороши, какие-то не очень, а некоторые вообще непривычны. Это руководство предназначено для того, чтобы дать последовательный и документированый подход в написание Sass кода.
...
[Первый коммит в Sass](https://github.com/hcatlin/sass/commit/fa5048ba405619273e474a50400c7243fbff54fe) был сделан в конце 2006, более 8 лет назад. Излишне говорить, что Sass прошел долгий путь с тех пор. Изначально разработанный на Ruby, получил множество портов там и тут. Самый популяный [LibSass](https://github.com/sass/libsass) (написанный на C) сейчас близок к полной совместимости с оригинальной Ruby версией.

В 2014 [Ruby Sass и LibSass команды решили подождать синхронизации версий, прежде чем двигаться дальше](https://github.com/sass/libsass/wiki/The-LibSass-Compatibility-Plan). С тех пор, LibSass активно выпускает версии, чтобы иметь контроль над четностью версий со старшим братом. Последние оставшиеся несоответствия собираются и перечисленные мною в проекте [Sass-совместимость](http://sass-compatibility.github.io). Если вы знаете о несовместимости между этими двумя версиями, которая не в списке, пожалуйста, будьте добры открыть обсуждение.

Возвращаясь к выбору компилятора. На самом деле, все зависит от вашего проекта. Если это Ruby on Rails проект, вам лучше использовать Ruby Sass, который идеально подходит для такого случая. Также следует помнить, что Ruby Sass — это всегда эталонная реализация и всегда будет обгонять LibSass в реализации.

На не Ruby проектах, которые нуждаются в интеграции, LibSass, вероятно, лучшая идея, поскольку является наиболее популярным. Так что если вы хотите использовать, скажем, NodeJS, [node-sass] (https://github.com/sass/node-sass) — ваш выбор.
...
Как видите, Sass первоначально описал синтаксис, определяющей характеристикой которого является его чувствительность к вложености. Вскоре в Sass решили сократить разрыв между Sass и CSS обеспечивая CSS-дружественный синтаксис под названием * SCSS * или * Sassy CSS *.Девиз: если это валидный CSS, то это и валидный SCSS.

С тех пор, Sass (препроцессор) предоставляет два различных синтаксиса: Sass (не все буквы заглавные, [пожалуйста](http://sassnotsass.com)), также известный как * синтаксис с отступом * и SCSS. Какой из них использовать в значительной степени зависит от вас, так как оба строго эквивалентны по возможностям. Это только вопрос эстетики.

Cинтаксис Sass опирается на отступы, чтобы избавиться от скобок, точки с запятой и других символов пунктуации, что приводит к более компактному и более короткому синтаксису. Между тем, SCSS легче учиться, так как это в основном крошечные дополнительные биты на вершине CSS.

Лично я предпочитаю SCSS над Sass, потому что он близок к CSS и более дружелюбен для большинства разработчиков. Поэтому, Я буду использовать SCSS в этом руководстве.
...
Sass — это препроцессор коих много. Самый серьезный соперник — это [LESS](http://lesscss.org/), препроцессор основанный на NodeJS, который стал весьма популярен благодаря CSS фреймворну [Bootstrap](http://getbootstrap.com/). Также есть [Stylus](http://learnboost.github.io/stylus/), который позволяет делать очень много, так как почти превращает CSS в язык программирования.

* Почему выбирать Sass, а не другой препроцессора? * Это все еще действующий вопрос сегодня. Не так давно, мы рекомендовали использовать Sass для проектов на Ruby, потому что Sass был сделан в Ruby и хорошо работал с Ruby On Rails. Теперь, когда LibSass догнал (в основном) оригинальный Sass, это уже не актуальный совет.

То, что мне нравится в Sass, так это его консервативный подход к CSS. Дизайн Sass основан на строгих принципах: большая часть проектного подхода приходит из мнений команды, что а) добавление дополнительных функций имеет стоимость сложности, которая должна быть оправдана полезностью и б) должен быть легко рассуждать о том, что данный блок стилей делает, глядя только на этот блок. Кроме того, Sass имеет гораздо более четкое внимание к деталям, чем другие препроцессоров. Насколько я могу судить, основные разработчики очень заботятся о поддержке совместимости с CSS и что каждое общее поведение является последовательным.

Другими словами, Sass не препроцессор, который направлен на удовлетворение гиковатых программистов вроде меня, добавляя функционал, который не предназначен для поддержки любых логических сценариев использования. Это программное обеспечение направленое на решение актуальных вопросов; помогает обеспечить полезную функциональность CSS, где CSS не дотягивает.

Препроцессоры в сторону, мы должны также упомянуть постпроцессоров, которые получили значительные дозы внимания в течение последних нескольких месяцев, главным образом благодаря [PostCSS] (https://github.com/postcss/postcss) и [cssnext] (HTTPS: // cssnext. github.io/). Постпроцессоры довольно много эквивалентны препроцессорам, кроме того что они не обеспечивают ничего, кроме синтаксиса CSS.

Вы можете думать о постпроцессоров как о полифиле для неподдерживаемых функций CSS. Например, можно было бы написать переменные, как они описаны в [спецификации CSS] (http://dev.w3.org/csswg/css-variables/), затем скомпилировать таблицы стилей с постпроцессором только, чтобы найти каждую переменную и заменить на соотвествующее значение, как бы Sass и сделал.

Идея постпроцессоров, что как только браузеры поддерживают новые функции (например, переменные CSS), постпроцессор не компилирует их больше, и позволяет браузеры взять это на себя.

Предоставлении синтаксиса завтрашнего дня является благородной идеей, я должен сказать, что я по-прежнему предпочитают использовать Sass для большинства задач. Тем не менее, есть некоторые случаи, когда я считаю, постпроцессоры больше подходят, чем Sass и как - CSS префикс, например, - но мы вернемся к этому.
...
## Почему руководство по стилям?

Руководство по стилям это не просто приятный документ для чтения, представляющий собой идеальное состояние для вашего кода. Это ключевой документ в жизни проекта, описания, как и почему код должен быть написан. Это может выглядеть как перебор для небольших проектов, но это очень помогает в сохранении кодовой базы чистой, масштабируемой и легкой в обслуживании.

Излишне говорить, что чем больше разработчиков, участвующих в проекте, тем больше руководств кода требуется. В том же ключе, чем больше проект, тем больше руководство по стилям, что является обязательным.

[Harry Roberts](http://csswizardry.com) обозначает это очень хорошо в [CSS Руководстве](http://cssguidelin.es/#the-importance-of-a-styleguide):

<blockquote>
  <p>Руководство по написанию стилей (заметьте, не визуальное) является ценным инструментом для команд, которые:</p>
  <ul>
    <li>создают и поддерживают продукты для разумного периода времени;</li>
    <li>имеют ряд разработчиков, различающихся способностей и специальностей;</li>
    <li>имеют ряд различных разработчиков, работающих на продуктом в любой момент времени;</li>
    <li>постоянная текучка кадров;</li>
...
Перво-наперво: ** это не CSS styleguide **. Этот документ не будет обсуждать именования классов CSS, модульных моделей и вопросов идентификаторов в CSS мире. Это руководство направлено только на борьбу с Sass содержанием.

Кроме того, это мое личное руководство стилей и поэтому ** очень субъективное **. Думайте об этом как о совокупности методологий и советов, которые я улучшал на протяжении многих лет. Это также дает мне возможность сделать перелинковку с проницательными ресурсами, так что не забудьте проверять * Дальнейшее чтениеs *.

Очевидно, что это, конечно, не единственный способ делать вещи, и это может или не может удовлетворить ваш проект. Не стесняйтесь брать отсюда код и адаптировать его под свои нужды. Как говорят у нас, * ваш пробег может варьироваться * (в том смысле, что в вашей ситуации может отличаться).
...
В конце концов,есть одна вещь, которую я бы хотел, чтобы вы получили от всего этого руководства, это то, что ** Sass должен быть максимально прост, как это может быть **.

Спасибо моим глупым эскпирементам, таким как [битовы операции](https://github.com/HugoGiraudel/SassyBitwise), [итераторы и генераторы](https://github.com/HugoGiraudel/SassyIteratorsGenerators) и [JSON парсер](https://github.com/HugoGiraudel/SassyJSON) на Sass, и теперь мы все хорошо знаем, что можно сделать с этим препроцессора.

Между тем, CSS является простым языкокм. Sass, предназначенный, чтобы написать CSS, не должен получаться гораздо сложнее, чем обычный CSS.[Принцип KISS,] (http://en.wikipedia.org/wiki/KISS_principle) (Keep It Simple Stupid) является ключевым моментом здесь и может даже взять верх над [принцепом DRY] (http://en.wikipedia.org / вики / Don% 27t_repeat_yourself) (не повторяться), в некоторых случаях.

Иногда, это лучше повторяться немного, чтобы держать код понятным, а не строить тяжелую, громоздкую, сложную систему, которая полностью не понятно и сложна в поддержке.

Кроме того, и позвольте мне процитировать [Гарри Робертс] (https://csswizardry.com) еще раз, ** прагматизм козырь совершенства **. В какой-то момент, вы, вероятно, пойдете против правил, описанных здесь. Если это имеет смысл — сделайте это. Код просто средство, а не конец.
...
Если вы спросите меня, самое первое, что руководство стилей должно сделать, это описать, как мы хотим, чтобы наши код смотрелся.

Когда несколько разработчиков участвуют в написание CSS проекта (ов), то это только вопрос времени, прежде чем один из них начинает делать вещи по-своему. Руководство стилей способствуют не только согласованости, но и помогают, когда время доходит то чтения и обновления кода.

Грубо говоря мы хотим (бесстыдно вдохновлен [CSS Guidelines](http://cssguidelin.es/#syntax-and-formatting)):
...
{% highlight sass %}
// С тех пор как Sass синтаксис заставляет использовать эти стандарты написания
...
Мы не будем решать вопрос организации файлов в этом разделе. Это объект архитектуры [другая секция](#architecture).
...
CSS не требует, чтобы строки были помещены в кавычки, даже те, содержащие пробелы. Возьмите названия семейства шрифтов, например: не имеет значения, обернуть их в кавычки для CSS парсера или нет.

Из-за этого, Sass * также * не требует помещения строк в кавычки. Даже лучше (и *, к счастью, *, ), строка в кавычках является точным эквивалентом его двойника без кавычек (например, `'abc'` строго равна `abc`).

Языки, которые не требуют, чтобы строки были в ковычках определенно в меньшинстве и так, ** строки должны всегда быть обернуты в одинарные кавычки ** в Sass (одну проще набрать, чем двойные на * QWERTY * клавиатуры). Кроме того, для согласованности с другими языками, в том числе с двоюродным братом CSS JavaScript'ом, есть несколько причин для такого выбора:

* названия цветов рассматриваются как цвета, когда без кавычек, может привести к серьезным проблемам;
* большинство синтаксических анализаторов будут в шоке от строк без кавычек;
* это помогает общей читаемости;
* нет правильной причины не обворачивать строки в ковычка.
...
<div class="note">
  <p>В предидущем примере, <code>sans-serif</code> не был обернут в ковычки потому что это специальное CSS значение, которые должно быть без ковычек.</p>
</div>

URLs тоже дожны быть в ковычках, по тем же причинам, что и выше:
